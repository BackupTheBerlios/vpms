...
const char* default_save_file = "save.dat";
const int ctrl_bytes = 285317550;
...
extern "C" SCM save_env(SCM fname) {
    FILE* f;
    if (scm_is_true(scm_string_p(fname))) {
        char* outfn = scm_to_locale_string(fname);
        f = fopen(outfn,"wb");
        free(outfn);
    }
    else {
        f = fopen(default_save_file,"wb");
    }

    if (f == 0) {
        vpms::log << "can not open output file for writing";
        return scm_from_bool(0);
    }

    if(vpms::env != 0) {
        fwrite(static_cast<const void*>(&ctrl_bytes),sizeof(unsigned int),1,f);
        vpms::cfg.Write(f);
        vpms::env->Clear();
        vpms::env->Write(f);
        fclose(f);
...
extern "C" SCM restore_env(SCM fname) {
    FILE* f;
    
    if (scm_is_true(scm_string_p(fname))) {
        char* infn = scm_to_locale_string(fname);
        f = fopen(infn,"rb");
        free(infn);
    }
    else {
        f = fopen(default_save_file,"rb");
    }

    if (f == 0) {
        vpms::log << "can not open input file for reading";
        return scm_from_bool(0);
    }

    int local_ctrl_bytes;
    fread(static_cast<void*>(&local_ctrl_bytes),sizeof(unsigned int),1,f);
    if(local_ctrl_bytes != ctrl_bytes) {
        vpms::log << "ERROR: control bytes does not match";
        return scm_from_bool(0);
    }
...
void vpms_main (void *closure, int argc, char **argv)
{
    ...
    scm_c_define_gsubr("save-environment",0,1,0,(SCM (*)())save_env);
    scm_c_define_gsubr("_restore-environment",0,1,0,(SCM (*)())restore_env);
